function generateEmailBody(issuesByOrg, totalIssues) {
...
function generateIssueRow(item, isLastIssue) {
...
async function searchAndNotify() {
  ...
  results.forEach((result) => {
    ...
    const issues = result.items
      .map((item, index) =>
        generateIssueRow(item, index === result.items.length - 1),
      )
      .join("");
  ...
  const totalIssues = issuesByOrg.reduce((sum, org) => sum + org.count, 0);

  const emailBody = generateEmailBody(issuesByOrg, totalIssues);
